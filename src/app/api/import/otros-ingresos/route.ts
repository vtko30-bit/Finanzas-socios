...
import { parseOtrosIngresosExcel } from "@/lib/import/excel";
...
export async function POST(request: Request) {
  try {
    ...
    const { data: previousBatch, error: previousBatchError } = await supabase
      ...
      .eq("status", "imported")
      .eq("summary_json->>importKind", "excel_otros_ingresos")
    ...
    if (previousBatch) {
      return NextResponse.json(
        {
          error:
            "Este archivo de otros ingresos ya fue importado. Si hiciste cambios, exporta un nuevo archivo antes de subirlo.",
          duplicateFile: true,
    ...
    let parsed: ReturnType<typeof parseOtrosIngresosExcel>;
    try {
      parsed = parseOtrosIngresosExcel(buffer);
    } catch (error) {
      return NextResponse.json(
        {
          error: `No se pudo procesar el Excel de otros ingresos: ${
    ...
    const { error: batchError } = await supabase.from("import_batches").insert({
      ...
      summary_json: {
        ...
        fileName,
        fileSize: file.size,
        importKind: "excel_otros_ingresos",
      },
      created_by: user.id,
    });
    ...
    return NextResponse.json(
      {
        error: `Error inesperado importando otros ingresos: ${
