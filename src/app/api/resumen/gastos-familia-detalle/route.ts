...
import { isoDateOk, loadGastosFamiliaCategoriaDetalle } from "@/lib/resumen-pivot-core";
...
export async function GET(request: Request) {
  ...
  const { data, error } = await loadGastosFamiliaCategoriaDetalle({
    supabase,
  ...
  if (error || !data) {
    return NextResponse.json({ error: error ?? "Error al cargar detalle" }, { status: 500 });
  }
