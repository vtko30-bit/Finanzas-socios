...
export function detectOrigenMaestroCode(
  ...
  if (n.includes("transferencias") || n.includes("transferencia")) {
    if (n.includes("chile") || n.includes("bci")) return ORIGEN_MAESTRO.trans_cl;
  ...
  if (movHint) {
    if (kind === "ingreso") {
      if (n.includes("bci") || n.includes("chile")) return ORIGEN_MAESTRO.ing_bci;
      if (n.includes("bancoestado") || n.includes("banco")) return ORIGEN_MAESTRO.ing_be;
    }
    if (n.includes("bci") || n.includes("chile")) return ORIGEN_MAESTRO.mov_bci;
...
export function etiquetaIngresoDesdeOrigenExcel(origenRaw: string): string {
  ...
  if (n.includes("bci") || n.includes("chile")) return "Bci";
