-- Dedupe por banco: BCI ≠ Banco de Chile. Cartola solo empareja con Transferencias del mismo banco.
...
as $$
  ...
  where t.organization_id = p_organization_id
    ...
    and not (
      ...
      and public.origen_familia_banco_dedupe(t.origen_cuenta::text) = 'be'
      and exists (
        select 1
        from public.transactions t2
        where t2.organization_id = t.organization_id
          ...
          and public.origen_familia_banco_dedupe(t2.origen_cuenta::text) = 'be'
          and (
            (
            ...
            or (t2.date = t.date and t2.amount = t.amount)
          )
      )
    )
    and not (
