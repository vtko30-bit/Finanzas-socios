-- TEF cartola vs Transferencias BE: detección laxa, Resumen alineado y limpieza en BD.

create or replace function public.es_fila_transferencias_be_dedupe(p_origen text)
returns boolean
language sql
immutable
as $$
  ...
  select
    (k like '%transferencias%' or k like '%transferencia%')
    and not (k like '%bci%' or k like '%chile%' or k like '%bancodechile%')
    and (
      k like '%banco%'
      or k like '%bestado%'
      or k like '%estado%'
      or k like '%bancoestado%'
      or k like '%rg'
    )
  from n;
$$;
    ...
    and not (
      ...
      and exists (
        ...
        where t2.organization_id = t.organization_id
          ...
          and lower(trim(t2.type::text)) in ('expense', 'gasto', 'egreso')
          and lower(trim(coalesce(t2.source::text, ''))) = 'excel_egresos'
          and public.es_fila_transferencias_be_dedupe(t2.origen_cuenta::text)
          and (
            (
              t2.date = t.date
...
-- Respaldo y eliminación de cartola TEF ya reflejada en Transferencias BE.
create table if not exists public.transactions_backup_tef_be_dup_20260527
(like public.transactions including all);

insert into public.transactions_backup_tef_be_dup_20260527
select t.*
from public.transactions t
...
where t.id in (select b.id from public.transactions_backup_tef_be_dup_20260527 b);
