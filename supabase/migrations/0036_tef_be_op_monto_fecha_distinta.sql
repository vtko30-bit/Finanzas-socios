-- TEF vs espejo BE: mismo N° operación + monto aunque la fecha difiera.
-- Corrige filas Transferencias importadas como Rg y limpia cartolas TEF duplicadas.
...
where lower(trim(coalesce(tef.source::text, ''))) = 'excel_egresos'
  and coalesce(tef.flow_kind, 'operativo') = 'operativo'
  and lower(trim(tef.type::text)) in ('expense', 'gasto', 'egreso')
  and upper(trim(coalesce(tef.description::text, ''))) like 'TEF%'
  ...
  and exists (
    ...
    where t2.organization_id = tef.organization_id
      ...
      and (
        ...
        or public.origen_familia_banco_dedupe(t2.origen_cuenta::text) = 'be'
      )
  );
...
-- Filas de hoja Transferencias BE que quedaron con origen Rg (p. ej. Id 135690875609).
...
where lower(trim(coalesce(t.source::text, ''))) = 'excel_egresos'
  ...
  and upper(trim(coalesce(t.description::text, ''))) not like 'TEF%'
  and exists (
    select 1
    from public.transactions tef
    where tef.organization_id = t.organization_id
      and tef.id <> t.id
      and coalesce(tef.flow_kind, 'operativo') = 'operativo'
      and lower(trim(tef.type::text)) in ('expense', 'gasto', 'egreso')
      and lower(trim(coalesce(tef.source::text, ''))) = 'excel_egresos'
      and upper(trim(coalesce(tef.description::text, ''))) like 'TEF%'
      and trim(coalesce(tef.external_ref::text, '')) = trim(coalesce(t.external_ref::text, ''))
      and round(abs(tef.amount::numeric), 2) = round(abs(t.amount::numeric), 2)
  );
